...
export default function AssetsPage({ rows = [], setRows, workOrders = [], onUpdateAsset }) {
  ...
  const updateAsset = (assetnum, patch) => {
    if (onUpdateAsset) onUpdateAsset(assetnum, patch)
    else setRows?.(current => current.map(row => row.assetnum === assetnum ? { ...row, ...patch } : row))
