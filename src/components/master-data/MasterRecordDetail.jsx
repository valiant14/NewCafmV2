    ...
    <section className="printable-record">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        {summary.map(([label, value]) => (
          <div key={label} className="rounded-[2rem] border border-[var(--app-line)] bg-[var(--app-panel)] p-4 shadow-[0_18px_50px_rgba(15,23,42,.06)]">
      ...
      <main className="grid gap-5 lg:grid-cols-2">
        {groups.map(group => (
          <section key={group.title} className="rounded-[2rem] border border-[var(--app-line)] bg-[var(--app-panel)] p-5 shadow-[0_18px_50px_rgba(15,23,42,.06)]">
            <header className="mb-4 flex items-center justify-between gap-3 border-b border-[var(--app-line)] pb-4">
              <div>
                <span className="text-[9px] font-extrabold uppercase tracking-[.16em] text-[var(--app-muted)]">{group.kicker}</span>
                <h2 className="text-base font-extrabold text-[var(--app-ink)]">{group.title}</h2>
              </div>
              <ChevronRight className="text-[var(--app-muted)]" size={17} />
            </header>
