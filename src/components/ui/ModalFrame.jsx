...
export function ModalOverlay({ children, className }) {
  return (
    <div className={cn('fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-[color:color-mix(in_srgb,var(--app-sidebar-bg)_72%,transparent)] p-6 backdrop-blur-sm', className)}>
      {children}
    </div>
  )
}
...
export function ModalPanel({ children, className, labelledBy }) {
  return (
    <section
      className={cn('flex max-h-[88vh] w-full flex-col overflow-hidden rounded-3xl border border-[var(--app-line)] bg-[var(--app-panel)] shadow-2xl', className)}
      aria-modal="true"
      role="dialog"
...
export function ModalHeader({ eyebrow, title, description, onClose, titleId, children, className }) {
  return (
    <header className={cn('flex items-start justify-between gap-4 border-b border-[var(--app-line)] bg-[var(--app-panel)] px-6 py-5', className)}>
      <div className="min-w-0">
        ...
        {title && <h2 className="mt-1 text-2xl font-extrabold tracking-[-.04em] text-[var(--app-ink)]" id={titleId}>{title}</h2>}
...
export function ModalFooter({ children, className }) {
  return (
    <footer className={cn('sticky bottom-0 flex items-center justify-end gap-2 border-t border-[var(--app-line)] bg-[var(--app-panel)] px-6 py-4', className)}>
      {children}
    </footer>
  )
}
