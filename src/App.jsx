...
function WorkOrderEditor({ order, onClose, page = false }) {
  ...
  const status = materialAvailable === 'Purchase Required' ? 'WAITMAT' : workflowStatus
  ...
  return <div className={page?'work-order-detail-page':'wo-overlay'}><div className={`wo-editor ${page?'work-order-page-editor':''}`}>
    ...
    <div className="wo-tab-help"><div><strong>{tab}</strong><span>{workOrderTabHelp[tab]}</span></div>{status==='WAPPR'&&<Badge tone="orange">Complete preparation before approval</Badge>}</div>
    <div className="wo-body">
      ...
      {tab==='PTW & Files' && <><Section title="Permit to Work (PTW)" note="Multiple documents supported"><div className="upload-zone"><Upload size={25}/><strong>Upload PTW documents</strong><span>PDF, DOCX, JPG or PNG · multiple files accepted</span><input type="file" multiple/></div></Section><Section title="General Attachments"><div className="attachment-row"><FileText/><div><strong>site-inspection-photo.jpg</strong><span>Uploaded today · 1.8 MB</span></div><button><X size={15}/></button></div></Section></>}
