...
const documentListClass = 'grid overflow-hidden rounded-2xl border border-[var(--app-line)] [&>div]:flex [&>div]:items-center [&>div]:gap-3 [&>div]:border-b [&>div]:border-[var(--app-line)] [&>div]:p-3 [&>div:last-child]:border-b-0 [&_strong]:text-sm [&_strong]:text-[var(--app-ink)] [&_small]:text-xs [&_small]:text-[var(--app-muted)]'
const documentActionsClass = 'ml-auto flex shrink-0 items-center gap-2'
const documentDownloadButtonClass = 'inline-flex h-8 items-center justify-center rounded-lg border border-[var(--app-line)] bg-[var(--app-panel)] px-3 text-xs font-bold text-[var(--app-muted)] transition hover:bg-[var(--app-soft-bg-hover)] hover:text-[var(--app-ink)]'
const documentRemoveButtonClass = 'inline-flex h-8 w-8 items-center justify-center rounded-lg text-[var(--app-muted)] transition hover:bg-[var(--app-badge-orange-bg)] hover:text-[var(--app-badge-orange-text)]'
...
function WorkOrderEditor({ order, onClose, page = false }) {
  ...
  </div><section className="wo-print-report"><h1>Work Order #{number}</h1><p>{description || order['DESCRIPITION '] || 'Work order'}</p><h2>Work Order Information</h2><div className="wo-print-grid">{[['Work Type',workType],['Status',status],['Priority',priority],['Site',siteValue],['Department',department],['Sub Department',subDepartment],['Assigned Department',assignedDepartment],['Location',locationValue],['Asset',assetValue],['Asset Description',assetDescription],['Target Start',targetStart],['Target Finish',targetFinish],['Actual Start',actualStart],['Actual Finish',actualFinish],['SLA Met?',slaLabel]].map(([label,value])=><div className="wo-print-cell" key={label}><span>{label}</span><strong>{value||'-'}</strong></div>)}</div><h2>Job Plan & Tasks</h2><ul className="wo-print-list">{plannedTasks.map((task,index)=><li key={index}><strong>{task.sequence || index + 1}. {task.description || 'Task instruction'}</strong><span> Duration: {task.duration || '-'} minutes</span></li>)}</ul><h2>Planned Labor / Materials / Tools</h2><div className="wo-print-grid">{plannedLabor.map((row,index)=><div className="wo-print-cell" key={`labor-${index}`}><span>Labor</span><strong>{row.craft || '-'} · {row.hours || '-'} hours</strong></div>)}{plannedResources.map((row,index)=><div className="wo-print-cell" key={`resource-${index}`}><span>{row.type}</span><strong>{row.item || '-'} · Qty {row.quantity || '-'}</strong></div>)}</div><h2>Failure & Actuals</h2><div className="wo-print-grid">{[['Failure Code',failureClass],['Problem Code',problemCode],['Cause Code',causeCode],['Remedy Code',remedyCode],['Technician Remarks',technicianRemarks],['Completion Notes',completionNotes],['Actual Labor',actualLabor],['Actual Hours',actualHours]].map(([label,value])=><div className="wo-print-cell" key={label}><span>{label}</span><strong>{value||'-'}</strong></div>)}</div></section></div>
}
