...
function WorkOrderEditor({ order, onClose, page = false }) {
  ...
  const planReady=Boolean(plannedLabor.some(row=>row.craft&&Number(row.hours)>0)&&plannedTasks.some(row=>row.description&&Number(row.duration)>0))
