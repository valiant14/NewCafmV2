...
const workOrderBodyClass = 'grid gap-4 px-5 pb-10 pt-0'
const workOrderTabsClass = 'flex overflow-auto border-b border-[var(--app-line)] bg-white'
const workOrderTabClass = active => [
  'relative flex items-center gap-1.5 whitespace-nowrap px-3 py-3 text-[11px] text-[#748078] transition hover:text-[var(--app-primary)]',
  active ? 'font-bold text-[#264737] after:absolute after:bottom-0 after:left-2.5 after:right-2.5 after:h-0.5 after:bg-[#477e63]' : ''
].join(' ')
...
function WorkOrderEditor({ order, onClose, page = false }) {
  ...
  return <div className={page?'work-order-detail-page space-y-5':'fixed inset-0 z-50 overflow-auto bg-[#112219]/70 p-6 backdrop-blur-sm'}><div className={page?'work-order-page-editor space-y-5':'mx-auto max-w-7xl space-y-5 rounded-3xl bg-[#fbfcfa] p-5 shadow-2xl'}>
    <header className="rounded-3xl border border-[var(--app-line)] bg-white p-6 shadow-[0_12px_32px_rgba(32,55,45,.07)]"><div><button className="mb-4 text-xs font-bold text-[#577066] transition hover:text-[var(--app-primary)]" onClick={close}>← Work Order Tracking</button><div className="flex flex-wrap items-center gap-3"><h2 className="text-3xl font-extrabold tracking-[-.045em] text-[var(--app-ink)]">{number === 'AUTO' ? 'New work order' : `Work order #${number}`}</h2><Badge tone={isPM?'blue':'purple'}>{workType}</Badge><Badge tone="orange">{status}</Badge></div><p className="mt-2 text-sm text-[var(--app-muted)]">{order['DESCRIPITION '] || order.DESCRIPTION || 'Enter work order information'}</p></div>
      <div className="flex flex-wrap items-center justify-end gap-2"><div className={`autosave-indicator ${autoSaveState.toLowerCase()}`}>{autoSaveState==='Saving'?<span className="saving-dot"/>:<Check size={13}/>}<span>{autoSaveState==='Saving'?'Saving…':'All changes saved'}</span></div><div className={`auto-status ${status.toLowerCase().replaceAll(' ','-')}`}><span>Automatic status</span><strong>{status}</strong></div>{status==='Waiting'&&<button className="primary complete-action" disabled={!overviewReady} onClick={()=>setWorkAssigned(true)}><Users size={15}/>Assign department</button>}{status==='ASSIGNED'&&<button className="primary complete-action" disabled={!preparationReady} onClick={()=>setWorkStarted(true)}><Wrench size={15}/>Start work</button>}{status==='INPRG'&&<button className="primary complete-action" disabled={!failureReady} onClick={completeWork} title={!failureReady?'Failure Code and Problem Code are required before completion':''}><Check size={15}/>Resolve / complete</button>}{status==='COMP'&&<button className="primary complete-action" disabled={!actualReady} onClick={()=>setWorkClosed(true)}><Check size={15}/>Close work order</button>}<button className="outline" onClick={reroute}><RotateCcw size={15}/> Re-route</button><button className="outline" onClick={()=>window.print()}><Printer size={15}/> Print</button><button className="close-editor" onClick={close}><X size={20}/></button></div></header>
    <div className={workOrderTabsClass}>{workOrderTabs.map((name,index)=><button key={name} className={workOrderTabClass(tab===name)} onClick={()=>setTab(name)}><small className="text-[8px] font-bold text-[#9aa39d]">{String(index+1).padStart(2,'0')}</small>{name}{name==='Failure'&&!isPM&&<i className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-[#e47b4d]"/>}</button>)}</div>
    <div className={workOrderBodyClass}>
      {tab==='Overview' && <><div className="wo-overview-layout"><div className="wo-overview-primary">
        <Section title="Work Order Information" note="Core information used throughout the work-order lifecycle"><div className={formGridClass}>
      ...
      {tab==='Completion & Actuals' && <>{!actualsEditable?<div className="actuals-locked"><div className="actuals-lock-icon"><ShieldCheck size={22}/></div><div><strong>Available after work completion</strong><p>{status==='ASSIGNED'?'Complete Plan and Failure preparation, then select Start Work. When execution is finished, select Resolve / Complete.':'When physical work is finished, select Resolve / Complete in the header to unlock execution notes and actual consumption.'}</p><span>Current status: {status}</span>{status==='ASSIGNED'&&!preparationReady&&<button className="outline locked-next" onClick={()=>setTab(planReady?'Failure':'Plan')}>Complete {planReady?'Failure':'Plan'} preparation</button>}{status==='ASSIGNED'&&preparationReady&&<button className="primary locked-next" onClick={()=>setWorkStarted(true)}>Start work</button>}{status==='INPRG'&&<button className="primary locked-next" onClick={completeWork}>Resolve / complete work</button>}</div></div>:<>
        <div className={`completion-timing ${slaBreachedNow?'breached':'met'}`}><div><span>Target Start<strong>{targetStart?new Date(targetStart).toLocaleString():'Not defined'}</strong></span><span>Target Finish<strong>{targetFinish?new Date(targetFinish).toLocaleString():'Not defined'}</strong></span><span>Actual Finish<strong>{actualFinish?new Date(actualFinish).toLocaleString():'Not recorded'}</strong></span></div><Badge tone={slaBreachedNow?'orange':'green'}>{slaLabel}</Badge></div><Section title="Execution Notes"><div className={twoColumnGridClass}><Field label="Technician Remarks" value={technicianRemarks} onChange={e=>setTechnicianRemarks(e.target.value)} type="textarea" required/><Field label="Completion Notes" value={completionNotes} onChange={e=>setCompletionNotes(e.target.value)} type="textarea" required/></div></Section>
        <Section title="Actual Labor"><div className={formGridClass}><Field label="Technician / Labor" value={actualLabor} onChange={e=>setActualLabor(e.target.value)} required/><Field label="Labor Craft Code" value={laborCraft} onChange={e=>setLaborCraft(e.target.value)} required/><Field label="Actual Labor Hours" value={actualHours} onChange={e=>setActualHours(e.target.value)} type="number" required/><Field label="Actual Start" value={actualStart} onChange={e=>setActualStart(e.target.value)} type="datetime-local"/></div></Section>
...
function ServiceRequestEditor({ request, onClose, onSubmit, onApprove }) {
  ...
  return <div className="w-full pb-5"><div className="mx-auto w-full max-w-[1400px] overflow-hidden rounded-3xl border border-[var(--app-line)] bg-[#f5f6f2] shadow-[0_12px_35px_rgba(32,55,45,.08)]"><header className="flex items-start justify-between gap-4 border-b border-[var(--app-line)] bg-white p-6"><div><button className="mb-3 text-xs font-bold text-[#577066] transition hover:text-[var(--app-primary)]" onClick={onClose}>← All Service Requests</button><div className="flex flex-wrap items-center gap-3"><h2>{form.sr==='AUTO'?'New service request':form.sr}</h2><Badge tone="orange">{form.status}</Badge></div><p>{isNew?'Tell us what happened and where. The maintenance team will configure the technical details.':'Review, configure, and convert this request without leaving the screen.'}</p></div><div className="flex flex-wrap items-center justify-end gap-2">{!isNew&&<button className="outline"><Printer size={15}/> Print</button>}</div></header>
    <div className="flex items-center justify-center bg-[#20372d] px-6 py-4 text-[9px] uppercase tracking-[.12em] text-[#90a299]"><span className="done">Request created</span><i className="mx-3 h-px w-16 bg-[#4a6156]"/><span className={form.status==='WAPPR'?'current':'done'}>Department review</span><i className="mx-3 h-px w-16 bg-[#4a6156]"/><span className={form.status==='CONVERTED'?'done':''}>CM work order</span></div>
    <div className="grid gap-4 p-5">
      {submitError&&<div className="flex items-center gap-3 rounded-xl border border-[#efc1aa] bg-[#fff3ec] p-3 text-sm text-[#a9532c]"><AlertTriangle size={17}/><span>{submitError}</span><button onClick={()=>setSubmitError('')}><X size={14}/></button></div>}
      <Section title={isNew?'Request Information':'Submitted Request'} note={isNew?'Provide the issue, location, and contact information.':'Original information submitted by the requester.'}><div className={srFormGridClass}>
        ...
        <div className="md:col-span-2"><Field label="Long Description" value={form.longDescription} type="textarea" onChange={update('longDescription')}/></div><Field label="Reported By" value={form.reportedBy} required onChange={update('reportedBy')}/><Field label="Reported Date & Time" value={form.reportedDate||now} type="datetime-local" locked/>
      </div></Section>
      <Section title="Attachments" note="Add photos or documents that help explain the request"><div className={uploadZoneClass}><Upload size={25}/><strong>Upload attachments</strong><span>Photos, PDFs and supporting documents · multiple files supported</span><input className="absolute inset-0 cursor-pointer opacity-0" type="file" multiple/></div></Section>
      {!isNew&&<Section title="Department Review & Configuration" note="Completed by the reviewing engineer or supervisor before CM conversion"><div className={srFormGridClass}><Field label="Department" value={form.department} required onChange={update('department')}/><Field label="Sub Department" value={form.subDepartment||''} onChange={update('subDepartment')}/><Field label="Assigned Department" value={form.assignedDepartment||form.department} required onChange={update('assignedDepartment')}/><Field label="Failure Code" value={form.failureCode} onChange={update('failureCode')} suggestions={failureClassOptions} placeholder="Search code or description"/></div></Section>}
      <div className={modeNoteClass('warning')}><AlertTriangle size={18}/><div><strong>{isNew?'Technical configuration happens after submission':'Ready for department review'}</strong><span>{isNew?'Department assignment and failure classification will be completed by maintenance staff.':'Confirm the department configuration before converting this request to CM.'}</span></div></div>
    </div><div className="sticky bottom-0 z-10 flex items-center justify-end gap-2 border-t border-[var(--app-line)] bg-white/95 p-4 backdrop-blur"><button className="outline" onClick={onClose}>Cancel</button><button className="primary sr-submit" onClick={handlePrimary} disabled={form.status==='CONVERTED'}><Check size={15}/>{form.status==='CONVERTED'?'Converted':isNew?'Submit request':'Approve & convert to CM'}</button></div></div></div>
}
