...
function WorkOrderEditor({ order, onClose, page = false }) {
  ...
  const overviewReady=Boolean(description.trim()&&siteValue&&assetValue&&locationValue&&department&&assignedDepartment)
  const overviewMissing=[!description.trim()&&'Description',!siteValue&&'Site',!assetValue&&'Asset',!locationValue&&'Location',!department&&'Department',!assignedDepartment&&'Assigned Department'].filter(Boolean)
  ...
  const planReady=Boolean(plannedLaborReady&&(isPM||(plannedMaterialsReady&&plannedToolsReady)))
  const failureReady=Boolean(isPM||(failureClass&&problemCode))
  const actualMaterialsReady=isPM||actualMaterials.some(row=>row.item&&Number(row.actualQuantity)>0)
  const actualToolsReady=isPM||actualTools.some(row=>row.item&&Number(row.actualQuantity)>0)
  ...
  useEffect(()=>{setAutoSaveState('Saving');const timer=setTimeout(()=>setAutoSaveState('Saved'),450);return()=>clearTimeout(timer)},[description,priority,department,subDepartment,assignedDepartment,workGroup,supervisor,laborCraft,siteValue,assetValue,locationValue,targetStart,targetFinish,failureClass,problemCode,causeCode,remedyCode,plannedLabor,plannedResources,plannedTasks,ptwRequired,ptwFiles,generalFiles,technicianRemarks,completionNotes,actualLabor,actualHours,actualMaterials,actualTools,actualStart,actualFinish,workAssigned,workStarted,workCompleted,workClosed])
