// Referential integrity audit for the mock data.
//
// Run: node scripts/audit-data.mjs   (exits non-zero on any broken link)
//
// The app has no backend, so every relationship - asset to location, PM to asset, craft to
// labour, material to store - lives in src/data as loose strings that nothing validates.
// This walks all of them and reports any reference that does not resolve. Run it after
// touching anything in src/data.
//
// Note: it deliberately does NOT flag the client's non-conforming asset codes
// (ALS-HV-00001 etc). Those resolve correctly; that they do not match the documented
// coding structure is a separate finding, reported in docs/02-location-asset-coding.md.
...
const J = n => require(join(here, '../src/data', n))
...
// src/lib/departments.js cannot be imported here - it uses a bare JSON import that plain
// Node rejects. Same resolution, rebuilt.
const allSubs=departments.flatMap(d=>d.subDepartments||[])
const subDepartmentName=v=>{const k=String(v??'').trim().toLowerCase();if(!k)return ''
  return allSubs.find(s=>s.code.toLowerCase()===k||s.name.toLowerCase()===k)?.name||String(v).trim()}
